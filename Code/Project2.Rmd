...
First, we can look at a basic summary and histograms of our target variable, the number of shares:
...
Another thing we can look at is the relationship between the number of shares and the number of shares in referenced articles (min, max, and average).
...
The standard practice is to use $m = p/3$, where $p$ represents the total number of predictors. For this case, that means we would use $m = 53/3 \approx 18$ after removing all non-predictive variables from the dataset. Thus, we will consider $m$ values ranging anywhere from - to -, selecting the optimal model based on the model performance in 10-fold cross-validation.

We will also ease the computational time by using parallel processing for this model, which requires a bit of setup first.
...
`names(best_model)` has the minimum MSE which indicates the best fitting.
